...
PrintMatrix(matrix);
WriteLine("Введите искомую позиции элемента в массиве через пробел: ");
int [] position = GetArrayForString(ReadLine()!);
if (position[0]>matrix.GetLength(0) || position[1]< 0 || position[1]>matrix.GetLength(1)|| position[1]<0)
{
    WriteLine("Данная позиция не находится в массиве");
    return;
}
WriteLine($"Искомый элемент равен: {matrix[position[0],position[1]]}");
